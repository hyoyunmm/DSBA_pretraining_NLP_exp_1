...
class IMDBDatset(torch.utils.data.Dataset):
    def __init__(self, data_config : omegaconf.DictConfig, split : Literal['train', 'valid', 'test']):
        ...
        test_frac  = float(getattr(data_config, 'test_frac',  0.1))
        ...
        first = merged.train_test_split(test_size=temp_frac, seed=self.seed, stratify_by_column="label")
        train_set = first["train"]
        temp_set  = first["test"]

        rel_test = test_frac / temp_frac
        second = temp_set.train_test_split(test_size=rel_test, seed=self.seed, stratify_by_column="label")
        valid_set, test_set = second["train"], second["test"]
    ...
    #@staticmethod
    ##### 동적패딩으로 수정 - 확인 필요
    def collate_fn(self, batch : List[Tuple[dict, int]]) -> dict:
        """
        Inputs :
            batch : List[Tuple[dict, int]]
        Outputs :
            data_dict : dict{
                input_ids : torch.Tensor (B, T_max)
                token_type_ids : torch.Tensor (B, T_max)
                attention_mask : torch.Tensor (B, T_max)
        ...
        # 동적 패딩 + 텐서 변환 
        padded = self.tokenizer.pad(enc_list, return_tensors="pt")  # pad_to_multiple_of = 8 처럼 텐서코어 효율 높일 수도 있는듯?

        if "token_type_ids" not in padded: # ModernBERT/RoBERTa 등은 token_type_ids가 없음 → 없으면 0으로 생성해 일관성 유지?
            padded["token_type_ids"] = torch.zeros_like(padded["input_ids"])
